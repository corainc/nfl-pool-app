...
module.exports = async function (context, req) {
    context.log('Processing request for user team stats...');
    ...
    try {
        ...
        // Query to fetch user team stats
        const result = await sql.query`
            ...
            JOIN
                TeamStats ts ON t.TeamID = ts.TeamID
            ORDER BY
                u.UserName,
                t.Name;
        `;
        context.log(`User team stats data retrieved: ${result.recordset.length} records found.`);
        ...
        context.log.error(`Error fetching user team stats: ${err.message}`);
        context.res = {
            status: 500,
            body: `Error fetching user team stats: ${err.message}`,
        };
    } finally {
